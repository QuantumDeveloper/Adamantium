...
3. **Форматы сверх `Text`/`Files`** (HTML, RTF, картинки, произвольные байты) + **deferred rendering**
   (отдавать тяжёлый формат лениво, в момент дропа).
4. **Анимация раздвигания соседей** под точкой вставки.
5. **Touch/pen** — за Pointer-абстракцией.

_(Закрытые пункты: «drag-handle», «`DragCursor`», «tunnel-варианты routed-событий»; «z-order при кросс-оконном
