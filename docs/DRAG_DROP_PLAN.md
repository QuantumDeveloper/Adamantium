...
2. **Форматы сверх `Text`/`Files`** (HTML, RTF, картинки, произвольные байты) + **deferred rendering**
   (отдавать тяжёлый формат лениво, в момент дропа).
3. **Анимация раздвигания соседей** под точкой вставки.
4. **Touch/pen** — за Pointer-абстракцией.

_(Закрытые пункты: «старт драга из кода», «drag-handle», «`DragCursor`», «tunnel-варианты routed-событий»; «z-order при кросс-оконном
