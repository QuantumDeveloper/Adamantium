...
- ✅ **`DragDrop`** (`Adamantium.UI/Input/DragDrop.cs` + `DragDrop.External.cs`, ОДИН статический класс на два
  ...
  - ❌ **`GiveFeedback` — НЕ реализован** (курсор драга выбирает движок по `Effects`; ручной override делается
    через `DragDropEventArgs.DragCursor` — он есть, см. «Routed-события»).
...
- **Нейтральные имена** (`DataFormats`): `Text` (string), `Files` (string[]), `Html` (string — пиши фрагмент,
  обёртку добавит платформа), `Rtf` (string), `Image` (**byte[] с ЗАКОДИРОВАННОЙ картинкой** — PNG, JPEG, GIF,
  BMP, TIFF, что было у источника; читать загрузчиком по СОДЕРЖИМОМУ, а не по имени формата, под которым приехало).
  Любое ДРУГОЕ имя — твой собственный формат: положи `byte[]`, и он поедет зарегистрированным
...
- **Windows-раскладка.** `Html` → `CF_HTML` с его заголовком из пяти БАЙТОВЫХ смещений (считаются по UTF-8, не
  ...
  есть). `Rtf` → зарегистрированный `Rich Text Format`, ANSI. `Image` уезжает ДВАЖДЫ: под своим собственным
  зарегистрированным именем — тем, которому байты РЕАЛЬНО соответствуют (`PNG`/`JFIF`/`GIF`/`TIFF`/`image/bmp`,
  выбирает `OleDataBridge.PictureFormat` по сигнатуре), — и как `CF_DIB` (Paint, Paint 3D, старый Office), который
  декодируется из тех же байт `Adamantium.Imaging`, строки СНИЗУ ВВЕРХ, каналы BGRA.
  **Ничего не перекодируется по дороге ни туда, ни обратно** — ни на выдаче, ни на приёме: перекодирование
  дороже всего остального в драге вместе взятого (анимированная гифка — это сотня мегапикселей кадров, секунды
  через наш кодировщик). Кодировка, которой у десктопа нет имени в буфере обмена (TGA, DDS), уезжает `CF_DIB`
  либо файлом.
...
- **На приём** читаются `Text`, `Files`, `Html`, `Rtf`, `Image`. Картинка берётся в ЛЮБОМ виде, который отдал
  источник (`PNG`/`JFIF`/`JPEG`/`GIF`/`TIFF`/`image/bmp`), и отдаётся вьюмодели ТЕМИ ЖЕ БАЙТАМИ — декодирует
  сторона-потребитель, `BitmapLoader` знает все эти форматы и определяет нужный по сигнатуре. Единственное
  исключение — сырой `CF_DIB`: это .bmp без своих первых 14 байт, поэтому дописываем файловый заголовок; это
  запись заголовка, а не конвертация. Формат, который не опознал ни один декодер, просто не сообщается —
  никаких исключений посреди дропа.
...
**Фаза 0, что осталось знать про `VisualRenderer`** (он общий, а не частный под призрак, и на нём же
будут стоять `VisualBrush`/`DrawingBrush` и превью дизайнера):

- **Печь живой on-screen элемент безопасно** — через ОТДЕЛЬНЫЙ `RenderCache`, живущий параллельно кэшу
  окна. Словари юнитов — поля экземпляра, поэтому коллизии нет, а уже-валидный компонент параллельный кэш
  просто читает, не пере-рендеривая, — общие марки не трогаются.
- **Время жизни одноразовое:** `IDisposable`, `create → render → (readback) → dispose`, без реконсиляции и
  attachment-цикла. Призраку отдаём CPU-bitmap и сразу освобождаем GPU-текстуру; превью и кистям — живой
  `RenderTargetImage`, которым владеет потребитель.

**Фаза 5, что важно помнить:** `WM_DROPFILES` (`WS_EX_ACCEPTFILES`) убран, вместо него `RegisterDragDrop` +
`IDropTarget` на HWND КАЖДОГО окна, один раз при создании и на всю жизнь окна. Чужой `IDataObject` читается
СРАЗУ в `DataPackage`, дальше путь тот же, что у in-app драга.
...
Из уровня 3 не сделаны только macOS/Linux — см. сводку в конце.
...
**DPI призрака.** Печём в `RenderScale` окна-источника (`_ghostBakeScale`), пере-масштабируем СОБЫТИЙНО по
`IDragGhost.DpiChanged` (хук `WM_DPICHANGED` через `WindowDpiWatcher`). Ключ, если полезешь туда снова:
ре-скейл считается ОТНОСИТЕЛЬНО масштаба бейка, иначе на high-DPI старте призрак раздувается вдвое
(bake × monitor). Хит-тест на мульти-мониторе живёт отдельно и проще: экранные координаты физические,
`PointToClient` делит на DPI-скейл окна.

**Живой призрак отложен осознанно.** Win DComp / Mac CAMetalLayer развели бы платформы; статичный битмап на
прозрачном окне ведёт себя на обеих одинаково. Возвращаться к этому — только если понадобится анимированный
призрак.

---

## Раздвигание при дропе (СДЕЛАНО, e090284)

Дыра на месте вставки вместо каретки, со своей карточкой-плейсхолдером, и плитки доезжают на новое
место, а не прыгают. Разбор оставлен целиком: каждое решение здесь выстрадано, а обратные варианты
выглядят соблазнительно.

**Что вышло:** `VirtualizingPanel.DropGapIndex` + `SlotOf`/`SlotCount`; `WrapPanel` и `StackPanel` укладывают по
СЛОТУ (у обоих слот выводится из индекса, поэтому дыра — одно сложение/умножение); `TryGetDropSlot` даёт индекс
из АРИФМЕТИКИ панели; `DropPlaceholderTemplate` — своя карточка (пунктирный акцентный контур в теме);
`AnimateLayoutMoves` — движение между двумя ответами раскладки, 0.18 с, память позиций ПО АЙТЕМУ;
выключатель `DragDrop.UseDropGap` (по умолчанию `True`), в демке — галочки «Spread».
**Грабли, на которые наступили (не наступать снова):**
- Индекс НЕЛЬЗЯ брать из положения контейнеров: дыра их двигает → меняется ближайший к курсору → меняется дыра.
  Живьём это выглядит как дрожание между двумя слотами. Тест `DropSlot_ComesFromTheGrid...` держит это.
- Анимировать ВСЯКОЕ перестроение раскладки нельзя: изменение размера окна меняет число колонок и гонит всю
  сетку (поймал `ViewportResize_Splices...`). Движение включается только на смену дыры.
- Панель нельзя искать через реализованный контейнер — в пустом списке их нет. И `GetVisualDescendants()`
  вопреки названию отдаёт только НЕПОСРЕДСТВЕННЫХ детей, так что нужен настоящий обход.
- В ПУСТОМ списке маркера вставки нет вовсе: каретка означает «между этими двумя», а там нет двух, и размер ей
  взять неоткуда (растягивалась во весь список). Цель обозначает подсветка области.
**Осталось по мелочи:** экстент прокрутки не учитывает открытую дыру у `StackPanel` (у `WrapPanel` учитывает
через `SlotCount`) — при перетаскивании в самый конец длинного списка последний элемент может уйти за границу.

_(Ниже — исходный разбор, по которому это делалось.)_

**Анимация раздвигания соседей** под точкой вставки — частный случай АНИМАЦИИ ЛЭЙАУТА, и делать надо
именно её: та же машинерия даст плавную вставку, удаление и сортировку в любом `ItemsControl`.

**Схема — лэйаут главный, трансформ только интерполирует.** Панель раскладывает детей по-настоящему, с
дырой нужного размера перед индексом вставки (панели передаётся «зарезервируй столько-то перед N», НЕ
плейсхолдер в коллекции — данные не трогаем). Значит во `WrapPanel` строка честно перетекает и крайняя
плитка честно уезжает на следующую строку. Анимируется только РАЗНИЦА между старой и новой позицией
ребёнка: `RenderTransform` стартует с «где я был» и едет в ноль. Так делала и старая WPF-демка
(`AnimatingTilePanel`), и наезда плиток при этом не бывает — финальные позиции настоящие, из `ArrangeOverride`.
Перекладка считается на СМЕНУ индекса вставки, а не на каждое движение мыши; между ними по кадрам работают
только трансформные анимации, а они дешёвые (`MarkTransform`, не структурная марка).

**Виртуализация не мешает, но диктует три правила.** (а) Анимируются только реализованные контейнеры;
уехавшее за экран встаёт сразу на финальное место — пользователь «до» и не видел. (б) Контейнеры
ПЕРЕИСПОЛЬЗУЮТСЯ, поэтому сдвиг обязан сбрасываться при рециклинге, а не только в конце жеста, иначе
получишь съехавшую строку вообще без драга. (в) **Предыдущая позиция помнится по АЙТЕМУ ДАННЫХ, а не по
контейнеру** — контейнер при прокрутке достаётся другому айтему, и сравнение «где был этот контейнер»
заставит плитки ездить из случайных мест при обычном скролле. Это главная ловушка fluid-layout поверх
виртуализации.

**В дыре стоит КАРТОЧКА, и это СВОЯ карточка, а не скелетон загрузки.** Размер берётся у СЛОТА — поэтому
вопрос «какого размера дыра» снимается сам, и дроп файла извне, чей размер заранее неизвестен, работает без
запасного варианта. Но шаблон нужен ОТДЕЛЬНЫЙ (`DropPlaceholderTemplate`), а не `ItemSkeletonTemplate`:
- Они означают РАЗНОЕ. Скелетон говорит «содержимое сейчас приедет», плейсхолдер — «сюда встанет то, что ты
  держишь». Одинаковый вид сливает два разных сообщения в одно.
- Скелетон кормит `IsLoadingItems`, а это шиммер на ВЕСЬ список: во время перетаскивания он сообщал бы о
  работе, которой нет. Своя карточка не трогает этот флаг вовсе — то есть «другой флаг» получается сам собой,
  как следствие разделения, а не как отдельная подпорка.
- У скелетона намеренная задержка в 6 кадров (чтобы не мигать на быстрой загрузке). Плейсхолдер под курсором
  обязан появляться мгновенно.
Пул и слот-машинерию `VirtualizingPanel` при этом переиспользуем — разделяется состояние, не механика.

**Каретка и раздвигание взаимоисключающи.** Не «каретка гаснет там, где получилось раздвинуть» — а либо одно,
либо другое: два сигнала об одном месте спорят друг с другом. Там, где раздвигание работает, каретки нет вовсе.

**Дефолт, а не опт-ин** — для любого `ItemsControl`, с выключателем через attached-свойство.

---

## Что осталось (сводка по состоянию на 2026-07-27)

**Единственный список несделанного — этот.** Всё, что не перечислено здесь, в коде есть; если встретишь
где-то выше «не сделано», а тут этого пункта нет — верна ЭТА сводка, а тот текст протух (и его надо снести,
а не дописать рядом ещё один статус).

1. **macOS / Linux-реализации** `INativeDragDrop`, `INativeDropSink`, `IDragGhost`. Готовы контракты и точка
   регистрации в оконном воркере; самих реализаций (AppKit `NSDraggingDestination` + `NSDraggingSession`,
   XDND / data-device Wayland) нет. Без них на этих платформах работает всё, кроме обмена с чужими приложениями.
2. **Touch/pen** — за Pointer-абстракцией.

_(Всё остальное из прежних «осталось» закрыто: форматы сверх Text/Files, deferred rendering, старт драга из
кода, drag-handle, `DragCursor`, tunnel-варианты routed-событий, z-order и подъём окна при кросс-оконном
драге.)_
