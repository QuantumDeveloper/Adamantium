...
2. **Анимация раздвигания соседей** под точкой вставки — частный случай АНИМАЦИИ ЛЭЙАУТА, и делать надо
   ...
   **Дыра — это СКЕЛЕТОН-карточка, а не абстрактный зазор.** Переиспользуем то, что уже есть:
   `ItemsControl.ItemSkeletonTemplate` + слот-машинерию `VirtualizingPanel` (она умеет измерять и раскладывать
   скелетон в слоте). Это заодно снимает вопрос «какого размера дыра»: размер даёт СЛОТ, поэтому дроп файла
   извне, чей размер заранее неизвестен, работает без выдумывания запасного варианта.
   *Но шиммер брать НЕЛЬЗЯ:* он висит на `IsLoadingItems` и означает «список догружает карточки». Дыра под
   дроп — не загрузка, и мерцающий во время перетаскивания список сказал бы пользователю, что идёт лишняя
   работа, которой нет. Нужен СВОЙ флаг на карточку-плейсхолдер; иначе шиммер перестанет означать хоть что-то.
