...
> **Единицы РАЗВЕДЕНЫ ТИПАМИ (2026-08-07).** Точка рабочего стола — `PixelPoint` (физические пиксели,
> `Adamantium.UI.Core`), всё внутри окна — `Vector2` (логические DIP). Переход только через `ToLogical(scale)` /
> `FromLogical(logical, scale)`, которые нельзя написать, не назвав масштаб; сложить одно с другим не компилируется.
> Раньше и то и другое было `Vector2`, разница держалась на комментариях — и терялась ровно там, где её не видно при
> 100%. Почему рабочий стол НЕ меряется в логических: при двух мониторах с разным масштабом «логическая позиция точки
> рабочего стола» не определена — та же точка переводится по-разному в зависимости от монитора. Физическое — то
> единственное описание, с которым согласны все мониторы, поэтому именно оно ходит между окнами, а перевод происходит
> у конкретного окна, по ЕГО масштабу. Подробности и что при этом всплыло — в `TECH_DEBT.md`.
