...
internal abstract class BatchArena
{
    ...
    /// <summary>Stop drawing this component's deferred fringe/stroke. Blanking its slots is only half of "it stopped
    /// drawing" for a family that defers ink to a per-unit draw: the shape goes, the INK stays, and a stroked path (a
    /// tab's close cross) leaves its stroke on the frame with nothing under it. Families that defer nothing say nothing.</summary>
    public virtual void DropOverlayOf(IUIComponent component) { }
