...
public class DataGridSortPanel : Control
{
    ...
    private readonly List<DataGridSortChip> _live = new();

    /// <summary>Lights up while a carried header is over the strip, so the drop reads as available before it happens.
    /// </summary>
    public static readonly AdamantiumProperty IsDropTargetProperty = AdamantiumProperty.Register(nameof(IsDropTarget),
        typeof(bool), typeof(DataGridSortPanel),
    ...
    /// <summary>The grid this strip sorts. Setting it REGISTERS the strip with that grid, which is what lets a change
    /// of sorting reach it.</summary>
    ...
    private void OnClear(object sender, RoutedEventArgs e) => Owner?.ClearSorting();
    ...
    protected override void OnMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        // The × is the way OUT of the sort, and the only one: a chip pressed anywhere else is being turned around or
        // picked up, and neither must unsort anything.
        if (_live[chip].PressedRemove(e.OriginalSource))
        {
            Owner?.RemoveSort(_live[chip].Column);
            e.Handled = true;
            return;
        }
    ...
    protected override void OnMouseLeftButtonUp(object sender, MouseButtonEventArgs e)
    {
        ...
        else if (_pressed >= 0 && _pressed < _live.Count)
        {
            // A press that went nowhere TURNS THE KEY AROUND. Unlike the grouping strip, where a chip has nothing to
            // say but its place, a sort key has a direction, and the chip that shows it is where it is changed.
            Owner?.FlipSort(_live[_pressed].Column);
        }
    ...
    /// <summary>Drops the carried chip where the mark stands, changing which key decides.</summary>
    internal void EndCarry(double x)
    {
        ...
        Owner?.MoveSorting(_dragging, target > _dragging ? target - 1 : target);
    ...
    internal void Sync()
    {
        ...
        var keys = Owner?.SortDescriptions;
        ...
        for (var i = 0; i < count; i++)
        {
            if (i == _live.Count)
            {
                var made = new DataGridSortChip();
                _live.Add(made);
                _chips.Children.Add(made);
            }

            _live[i].Attach(keys[i], i + 1);
        }
