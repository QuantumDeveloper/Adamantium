...
public partial class TreeDataGrid : Selector
{
    ...
    /// <summary>The member a record is known BY, when the tree is a flat list joined on keys rather than records
    /// holding their own children. Set with <see cref="ParentKeyPath"/>; either alone says nothing.
    /// <para>This is the shape data arrives in from a database or a service - rows with an id and a parent id - and it
    /// was the one kind of tree this table could only be told about in code.</para></summary>
    public static readonly AdamantiumProperty KeyPathProperty = AdamantiumProperty.Register(nameof(KeyPath),
        typeof(String), typeof(TreeDataGrid), new PropertyMetadata(null, PropertyMetadataOptions.AffectsMeasure,
            OnRelationChanged));

    public String KeyPath
    {
        get => GetValue<String>(KeyPathProperty);
        set => SetValue(KeyPathProperty, value);
    }

    /// <summary>The member holding the key of the record this one belongs to. A record naming none, naming one no
    /// record in the list answers to, or naming itself, stands at the top.</summary>
    public static readonly AdamantiumProperty ParentKeyPathProperty = AdamantiumProperty.Register(
        nameof(ParentKeyPath), typeof(String), typeof(TreeDataGrid),
        new PropertyMetadata(null, PropertyMetadataOptions.AffectsMeasure, OnRelationChanged));

    public String ParentKeyPath
    {
        get => GetValue<String>(ParentKeyPathProperty);
        set => SetValue(ParentKeyPathProperty, value);
    }

    private static void OnRelationChanged(AdamantiumComponent d, AdamantiumPropertyChangedEventArgs e) =>
        (d as TreeDataGrid)?.Refresh();

    // Both, or neither: one path alone cannot say what belongs to what, and half a relation quietly doing nothing is
    // the kind of no-op nobody finds.
    private bool HasKeyRelation =>
        KeyPath is { Length: > 0 } && ParentKeyPath is { Length: > 0 } && ChildrenSelector == null;

    private DataGridKeyRelation _relation;
    ...
    {
        // A relation made of keys is the whole TREE, not one row's place in it: a record arriving or leaving changes
        // what belongs to what, and the lookup that says so was worked out before it happened.
        if (Filter != null || HasColumnFilters || SortColumn != null || HasKeyRelation)
        {
            RebuildFlattener();
            return;
        }
    ...
    private void RebuildFlattener(bool sameRows = false)
    {
        ...
        // opens one.
        _relation = HasKeyRelation && _roots != null
            ? DataGridKeyRelation.Build(_roots, KeyPath, ParentKeyPath)
            : null;
    ...
    /// <summary>The keys the rows are sorted by, in PRIORITY order: the first decides, the next breaks its ties. The
    /// collection IS the mechanism - a page states its sort here, the strip shows it, and both are looking at the one
    /// list rather than at two ideas of what the table is sorted by.</summary>
    public DataGridSortDescriptions SortDescriptions { get; } = new();

    /// <summary>The column the rows are sorted by FIRST, or null for the source's own order. The one-key view of
    /// <see cref="SortDescriptions"/>, kept because most tables have exactly one key and most callers only ever ask
    /// about it.</summary>
    public DataGridColumn SortColumn => SortDescriptions.Count > 0 ? SortDescriptions[0].Column : null;

    /// <summary>Whether <see cref="SortColumn"/> runs the other way.</summary>
    public bool SortDescending => SortDescriptions.Count > 0 && SortDescriptions[0].Descending;

    /// <summary>Where this column stands in the sort, from 1, or 0 when it is not one of the keys. What the header
    /// shows beside its arrow - with two keys and no number, an arrow on each says nothing about which decides.
    /// </summary>
    public int SortLevelOf(DataGridColumn column)
    {
        for (var i = 0; i < SortDescriptions.Count; i++)
        {
            if (ReferenceEquals(SortDescriptions[i].Column, column)) return i + 1;
        }
    ...
    public void SortBy(DataGridColumn column, bool descending = false)
    {
        ...
        try
        {
            ...
            if (column is { CanSort: true }) SortDescriptions.Add(new DataGridSortDescription(column, descending));
        }
        finally
        {
            _resorting = false;
    ...
    /// sorted by and breaks its ties with this - which is the whole of what a second key is for.</summary>
    public void AddSort(DataGridColumn column, bool descending = false)
    {
        if (column is not { CanSort: true }) return;

        var at = SortLevelOf(column) - 1;
        if (at >= 0) SortDescriptions[at] = new DataGridSortDescription(column, descending);
        else SortDescriptions.Add(new DataGridSortDescription(column, descending));
    }

    /// <summary>Takes one key out. The keys under it move up; the table stays sorted by the rest.</summary>
    public void RemoveSort(DataGridColumn column)
    {
        var at = SortLevelOf(column) - 1;
        if (at >= 0) SortDescriptions.RemoveAt(at);
    }

    /// <summary>Turns one key around without changing where it stands in the order.</summary>
    public void FlipSort(DataGridColumn column)
    {
        var at = SortLevelOf(column) - 1;
        if (at < 0) return;

        SortDescriptions[at] = new DataGridSortDescription(column, !SortDescriptions[at].Descending);
    }

    /// <summary>Moves a key to another place in the order - the FIRST one decides, so this is what changes which
    /// column the table is really sorted by.</summary>
    public void MoveSorting(int from, int to)
    {
        if (from < 0 || from >= SortDescriptions.Count || to < 0 || to >= SortDescriptions.Count || from == to) return;

        var key = SortDescriptions[from];

        _resorting = true;
        try
        {
            SortDescriptions.RemoveAt(from);
            SortDescriptions.Insert(to, key);
        }
        finally
        {
            _resorting = false;
    ...
    /// <summary>Back to the source's own order.</summary>
    ...
    // A rewrite made of several steps announces each of them, and re-sorting ten thousand rows per step is the same
    // answer bought three times over. The steps that come in pairs say so and sort once at the end.
    private bool _resorting;

    private void OnSortDescriptionsChanged(object sender, NotifyCollectionChangedEventArgs e)
    {
        if (!_resorting) ApplySorting();
    }
    ...
    /// <summary>Whether the strip that says what the table is sorted by is shown. It is the only explicit way to sort
    /// by more than one column - and the only place the ORDER of the keys can be seen at all.</summary>
    public static readonly AdamantiumProperty ShowSortPanelProperty = AdamantiumProperty.Register(
        nameof(ShowSortPanel), typeof(bool), typeof(TreeDataGrid),
        new PropertyMetadata(false, PropertyMetadataOptions.AffectsMeasure, OnShowSortPanelChanged));

    public bool ShowSortPanel
    {
        get => GetValue<bool>(ShowSortPanelProperty);
        set => SetValue(ShowSortPanelProperty, value);
    }

    // Taking the strip away leaves the FIRST key and drops the rest - unlike the grouping strip, which unsorts
    // altogether. Sorting by one column is something the headers can say on their own and the user can undo; being
    // sorted by three with no strip to show it is a table in an order nothing explains.
    private static void OnShowSortPanelChanged(AdamantiumComponent d, AdamantiumPropertyChangedEventArgs e)
    {
        if (d is not TreeDataGrid { ShowSortPanel: false } grid || grid.SortDescriptions.Count <= 1) return;

        grid._resorting = true;
        try
        {
            while (grid.SortDescriptions.Count > 1) grid.SortDescriptions.RemoveAt(grid.SortDescriptions.Count - 1);
        }
        finally
        {
            grid._resorting = false;
        }
    ...
    private IEnumerable Shape(IEnumerable source)
    {
        ...
        if (!filtering && SortDescriptions.Count == 0) return source;
        ...
        if (SortDescriptions.Count == 0) return items;
        ...
        // 889 ms to sort ten thousand. THREE keys must not mean three times that: every key is read once per row, up
        // front, and what the comparison walks afterwards is the readings.
        var rows = items.ToArray();
        var keys = new object[SortDescriptions.Count][];
        for (var k = 0; k < keys.Length; k++)
        {
            var column = SortDescriptions[k].Column;
            var read = new object[rows.Length];
            for (var i = 0; i < rows.Length; i++) read[i] = ValueOf(column, rows[i]);
            keys[k] = read;
        }

        // One key is the overwhelming case, and it sorts exactly as it always did - no indirection bought for a
        // tie-break that is not there.
        if (keys.Length == 1)
        {
            Array.Sort(keys[0], rows, SortDescriptions[0].Descending ? Descending : Ascending);
            return rows;
        }

        // The ORDER is sorted, not the rows and not the readings: several key arrays cannot be carried along by
        // Array.Sort, so what moves is a list of positions into all of them.
        var order = new int[rows.Length];
        for (var i = 0; i < order.Length; i++) order[i] = i;

        var descending = new bool[keys.Length];
        for (var k = 0; k < keys.Length; k++) descending[k] = SortDescriptions[k].Descending;

        Array.Sort(order, (a, b) =>
        {
            for (var k = 0; k < keys.Length; k++)
            {
                var verdict = Ascending.Compare(keys[k][a], keys[k][b]);
                if (verdict != 0) return descending[k] ? -verdict : verdict;
            }

            // Equal on every key: the source's own order decides, so a re-sort does not shuffle what it cannot tell
            // apart.
            return a.CompareTo(b);
        });

        var sorted = new object[rows.Length];
        for (var i = 0; i < order.Length; i++) sorted[i] = rows[order[i]];
        return sorted;
    }
    ...
    public DataGridColumnsState CaptureColumnState()
    {
        ...
        foreach (var key in SortDescriptions)
        {
            if (key.Column?.StateKey is { Length: > 0 } saved)
            {
                state.Sort.Add(new DataGridSortKeyState { Key = saved, Descending = key.Descending });
            }
        }
    ...
    public void RestoreColumnState(DataGridColumnsState state)
    {
        ...
        // The LIST first, and the one-key form only when there is no list: a file written before the table could sort
        // by several carries the old fields alone, and coming back unsorted from it would be the saving quietly
        // failing.
        if (state.Sort is { Count: > 0 })
        {
            _resorting = true;
            try
            {
                SortDescriptions.Clear();
                foreach (var key in state.Sort)
                {
                    if (key?.Key is { Length: > 0 } named && byKey.TryGetValue(named, out var by) && by.CanSort)
                    {
                        SortDescriptions.Add(new DataGridSortDescription(by, key.Descending));
                    }
                }
            }
            finally
    ...
    internal void AdoptSortPanel(DataGridSortPanel panel) => _sortPanel = panel;

    private DataGridSortPanel _sortPanel;
    ...
    internal bool IsOverSortPanel(IUIComponent from, Vector2 point)
    {
        if (_sortPanel == null || !ShowSortPanel || from == null) return false;

        var local = from.TranslatePoint(point, _sortPanel);
        var size = _sortPanel.RenderSize;
        return local.X >= 0 && local.Y >= 0 && local.X <= size.Width && local.Y <= size.Height;
    }

    /// <summary>Which strip a carried header is over, so the band that answers the drop and the band that lights up
    /// are decided in ONE place. Asked on every move of a carry, which is why it is an index and not a search.</summary>
    internal DataGridDropStrip StripUnder(IUIComponent from, Vector2 point)
    {
        if (IsOverGroupPanel(from, point)) return DataGridDropStrip.Grouping;
        if (IsOverSortPanel(from, point)) return DataGridDropStrip.Sorting;
        return DataGridDropStrip.None;
    }

    /// <summary>Marks one strip as the place a carried header would land, and takes the mark off the other.</summary>
    internal void MarkDropStrip(DataGridDropStrip strip)
    {
        MarkGroupPanelTarget(strip == DataGridDropStrip.Grouping);
        if (_sortPanel != null) _sortPanel.IsDropTarget = strip == DataGridDropStrip.Sorting;
    }
    ...
    public override void OnApplyTemplate()
    {
        ...
        if (GetTemplateChild("PART_SortPanel") is DataGridSortPanel sortPanel) sortPanel.Owner = this;
