...
public class DataGridColumnHeader : ContentControl
{
    ...
    /// <summary>Where this column stands in the sort, from 1, or 0 when there is nothing to number - not a key, or the
    /// only one. With several keys and no number an arrow on each says nothing about which of them decides.</summary>
    public static readonly AdamantiumProperty SortLevelProperty = AdamantiumProperty.Register(nameof(SortLevel),
        typeof(int), typeof(DataGridColumnHeader),
        new PropertyMetadata(0, PropertyMetadataOptions.AffectsRender));

    public int SortLevel
    {
        get => GetValue<int>(SortLevelProperty);
        set => SetValue(SortLevelProperty, value);
    }

    /// <summary>The number as the theme shows it, or empty when there is none - a template binds text, not an int it
    /// would have to hide at zero.</summary>
    public static readonly AdamantiumProperty SortLevelTextProperty = AdamantiumProperty.RegisterReadOnly(
        nameof(SortLevelText), typeof(string), typeof(DataGridColumnHeader),
        new PropertyMetadata(string.Empty, PropertyMetadataOptions.AffectsRender));

    public string SortLevelText
    {
        get => GetValue<string>(SortLevelTextProperty);
        private set => SetValue(SortLevelTextProperty, value);
    }
