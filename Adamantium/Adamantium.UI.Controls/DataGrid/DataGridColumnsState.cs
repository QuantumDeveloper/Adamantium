...
public sealed class DataGridColumnsState
{
    ...
    /// <summary>Every key the table was sorted by, in priority order.</summary>
    public List<DataGridSortKeyState> Sort { get; set; } = new();

    /// <summary>The key of the column the table was sorted by, or null for no sort. The ONE-KEY form, written as well
    /// as <see cref="Sort"/> and read when that is empty, so a file saved before the table could sort by several still
    /// restores rather than coming back unsorted.</summary>
    public string SortKey { get; set; }

    public bool SortDescending { get; set; }
}

/// <summary>One key of a saved sort: which column, and which way it ran.</summary>
public sealed class DataGridSortKeyState
{
    /// <summary>Which column this is about - <see cref="DataGridColumn.StateKey"/>.</summary>
    public string Key { get; set; }
