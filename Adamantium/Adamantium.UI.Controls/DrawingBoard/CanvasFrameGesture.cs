...
public sealed class CanvasFrameGesture
{
    ...
    private double _leastWidth;
    private double _leastHeight;
    ...
    public void Begin(InfiniteCanvas canvas, CanvasHandle handle, Vector2 world)
    {
        ...
        // HOW FAR THE FRAME MAY BE SQUEEZED, worked out once from what is being held. Everything is scaled by the same
        // factor, so the frame may not go below the factor the neediest item can still bear: told to be smaller, that
        // one would spill out of a frame drawn round it, which is what made a node collapse to a sliver with its own
        // title hanging out of the side.
        var shrinkX = 0.0;
        var shrinkY = 0.0;
        ...
        {
            var box = item.Bounds;
            _startBounds.Add(box);

            var least = item.Smallest;

            if (box.Width > 1e-9 && least.Width > 0) shrinkX = Math.Max(shrinkX, least.Width / box.Width);
            if (box.Height > 1e-9 && least.Height > 0) shrinkY = Math.Max(shrinkY, least.Height / box.Height);
        }

        _leastWidth = _startFrame.Width * shrinkX;
        _leastHeight = _startFrame.Height * shrinkY;
    }
    ...
    public void MoveTo(InfiniteCanvas canvas, Vector2 world)
    {
        ...
        {
            var hair = canvas.ScreenToWorldLength(2);

            Apply(canvas, Stretch(_startFrame, Handle, delta,
                Math.Max(hair, _leastWidth), Math.Max(hair, _leastHeight)));
        }
    ...
    private static Rect Stretch(Rect from, CanvasHandle grip, Vector2 delta, double leastWidth, double leastHeight)
    {
        var left = from.X;
        var top = from.Y;
