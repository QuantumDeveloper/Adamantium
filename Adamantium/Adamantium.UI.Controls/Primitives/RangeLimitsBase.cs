...
/// the same limits, the same step sizes, and the same rule that moving a limit re-checks whatever is selected - which is
/// what lives here, rather than being written twice.
/// </summary>
public abstract class RangeLimitsBase : Control
{
    public static readonly AdamantiumProperty MinimumProperty = AdamantiumProperty.Register(nameof(Minimum),
        typeof(double), typeof(RangeLimitsBase), new PropertyMetadata(0.0, OnMinimumChanged));

    public static readonly AdamantiumProperty MaximumProperty = AdamantiumProperty.Register(nameof(Maximum),
        typeof(double), typeof(RangeLimitsBase), new PropertyMetadata(1.0, OnMaximumChanged));

    public static readonly AdamantiumProperty SmallChangeProperty = AdamantiumProperty.Register(nameof(SmallChange),
        typeof(double), typeof(RangeLimitsBase), new PropertyMetadata(1.0));

    public static readonly AdamantiumProperty LargeChangeProperty = AdamantiumProperty.Register(nameof(LargeChange),
        typeof(double), typeof(RangeLimitsBase), new PropertyMetadata(10.0));

    /// <summary>Lower limit. Anything selected below this clamps up.</summary>
    ...
    /// <summary>Upper limit. Kept &gt;= Minimum by coercing what is selected (the limits themselves are not reordered).</summary>
    ...
    protected static void OnMinimumChanged(AdamantiumComponent d, AdamantiumPropertyChangedEventArgs e)
    {
        ...
        var range = (RangeLimitsBase)d;
        range.ReCoerceSelection();
        range.OnLimitsChanged();
    }
    ...
    /// <summary>Re-run the coercion of whatever this control selects - a limit just moved and may have left it outside.
