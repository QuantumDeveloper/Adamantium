...
namespace Adamantium.Engine.Graphics.Imaging.PaletteQuantizer.Quantizers.MedianCut
{
    internal class MedianCutCube
    {
        ...
        public Color Color
        {
            get
            {
                ...
                foreach (Int32 rgba in colorList)
                {
                    Color color = Color.FromRgba(rgba);
                ...
                Color result = Color.FromRgba((byte)red, (byte)green, (byte)blue, 255);
                return result;
            }
        }
        ...
        public void SplitAtMedian(Byte componentIndex, out MedianCutCube firstMedianCutCube, out MedianCutCube secondMedianCutCube)
        {
            ...
            switch (componentIndex)
            {
                // red colors
                case 0:
                    colors = colorList.OrderBy(rgba => ColorModelHelper.GetComponentA(ColorModel, Color.FromRgba(rgba))).ToList();
                    break;

                // green colors
                case 1:
                    colors = colorList.OrderBy(rgba => ColorModelHelper.GetComponentB(ColorModel, Color.FromRgba(rgba))).ToList();
                    break;

                // blue colors
                case 2:
                    colors = colorList.OrderBy(rgba => ColorModelHelper.GetComponentC(ColorModel, Color.FromRgba(rgba))).ToList();
                    break;
