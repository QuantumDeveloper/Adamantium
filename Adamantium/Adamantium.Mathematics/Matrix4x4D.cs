...
namespace Adamantium.Mathematics
{
   public struct Matrix4x4D
   {
      ...
      public Matrix4x4D(double[] values)
      {
         ...
         if (values.Length < 16)
            throw new ArgumentOutOfRangeException(nameof(values), "There must be sixteen and only sixteen input values for Matrix4x4D.");
      ...
      public double this[int index]
      {
         ...
         set
         {
            switch (index)
            {
               ...
               default: throw new ArgumentOutOfRangeException("index", "Indices for Matrix4x4D run from 0 to 15, inclusive.");
            }
         }
      }
