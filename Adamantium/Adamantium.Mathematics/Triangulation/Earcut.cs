// This is a C# port of mapbox/earcut (https://github.com/mapbox/earcut), used under the ISC license:
//
//   ISC License
...
namespace Adamantium.Mathematics.Triangulation
{
    ...
    /// (self-intersecting) input — which is why it suits glyph/SVG data.
    /// </summary>
    internal static class Earcut
    {
        public static List<int> Tessellate(double[] data, int[] holeIndices, int dim = 2)
        {
            ...
            // z-order curve hashing pays off only for larger polygons
        ...
        private static Node FilterPoints(Node start, Node end = null)
        {
            ...
            {
                again = false;
                if (!p.steiner && (PointsEqual(p, p.next) || Area(p.prev, p, p.next) == 0))
                {
                    RemoveNode(p);
                    p = end = p.prev;
                    if (p == p.next) break;
        ...
        private static Node CureLocalIntersections(Node start, List<int> triangles, int dim)
        {
            ...
            do
            {
                ...
                if (!PointsEqual(a, b) && Intersects(a, p, p.next, b) && LocallyInside(a, b) && LocallyInside(b, a))
                {
        ...
        {
            int xx = (int)((x - minX) * invSize);
            int yy = (int)((y - minY) * invSize);
        ...
        private static bool IsValidDiagonal(Node a, Node b)
        {
            return a.next.i != b.i && a.prev.i != b.i && !IntersectsPolygon(a, b) &&
                   (LocallyInside(a, b) && LocallyInside(b, a) && MiddleInside(a, b) &&
                    ...
                    PointsEqual(a, b) && Area(a.prev, a, a.next) > 0 && Area(b.prev, b, b.next) > 0);
        }
        ...
        private static bool PointsEqual(Node p1, Node p2)
        {
