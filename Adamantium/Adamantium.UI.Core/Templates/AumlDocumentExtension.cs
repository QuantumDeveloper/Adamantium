...
public static class AumlDocumentExtension
{
    public static AumlTemplateContainer TransformAumlDocument(this AumlDocument document)
    {
        ...
        while (stack.Count > 0)
        {
            ...
            if (string.IsNullOrEmpty(element.TypeReference.Namespace))
            {
                var types = allTypes.Where(t => t.Name == element.TypeReference.Name);
                ...
                element.TypeReference = clrType;
                metaContainer.TypesMap[finalType.FullName] = finalType;
            }
            else if (!metaContainer.TypesMap.TryGetValue(element.TypeReference.Namespace, out var controlType))
            {
                controlType = Type.GetType(element.TypeReference.GetFullTypeName());
                metaContainer.TypesMap[element.TypeReference.Namespace] = controlType;
            }
